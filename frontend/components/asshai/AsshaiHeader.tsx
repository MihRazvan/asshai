...
import { ChevronDown, History, Sparkles, Wallet } from "lucide-react";
...
export function AsshaiHeader() {
  ...
  return (
    <header className="relative z-20 mx-auto flex w-full max-w-[88rem] items-center justify-between px-5 py-5 lg:px-8">
      <AsshaiWordmark />
      <DropdownMenu>
        <DropdownMenuTrigger className="hidden items-center gap-2 rounded-full border border-white/[0.1] bg-white/[0.035] px-4 py-2 font-mono text-xs uppercase tracking-[0.22em] text-white/70 outline-none transition-colors hover:border-accent/35 hover:text-white md:inline-flex">
          Intent compiler
          <ChevronDown size={14} />
        </DropdownMenuTrigger>
        <DropdownMenuContent
        ...
        >
          <DropdownMenuItem
            className="gap-2 rounded-lg px-3 py-2 text-white/78 focus:bg-accent/15 focus:text-white"
            onClick={() => router.push("/")}
          >
