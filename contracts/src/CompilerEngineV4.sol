...
contract CompilerEngineV4 {
    ...
    function handleRatesResponse(uint256 requestId, Response[] memory responses, ResponseStatus status, Request memory)
        external
    {
    ...
    function _requestDecision(uint256 goalId, string memory compactPoolData) private {
        ...
        string memory prompt =
            _buildDecisionPrompt(goal.naturalLanguage, _joinConstraints(goal.constraints), compactPoolData);
    ...
    function _promptIntro() private pure returns (string memory) {
        return string.concat(
    ...
    function _promptRules() private pure returns (string memory) {
        return string.concat(
            "Selection rules: if the goal says safe, safest, conservative, low risk, or no sketchy pools, ",
            "choose the lowest riskTier venue, even if APY is lower. ",
            "If the goal says max, maximize, best yield, highest yield, or best return, choose the highest APY verified venue ",
            "unless the user also explicitly prioritizes safety. ",
            "If the user names a supported protocol or venue, choose that pool if it appears in Candidates. ",
            "If the goal asks for vaults, managed vaults, Morpho, Moonwell, Fluid, or Steakhouse, prefer matching vault routes. ",
            "If the goal requests an APY above all candidates, choose the highest APY verified non-sketchy pool and set objectiveMatched to fallback. ",
            "Return ONLY compact JSON. No markdown. "
        );
    }

    function _promptSchema() private pure returns (string memory) {
        return string.concat(
            "Schema: ",
            "{\"poolId\":\"<id>\",\"objectiveMatched\":\"<max_yield|safety|protocol_preference|fallback>\",",
    ...
    function _isSupportedPool(string memory poolId) private pure returns (bool) {
        return _stringEq(poolId, "aave-v3-usdc-base") || _stringEq(poolId, "compound-v3-usdc-base")
            || _stringEq(poolId, "morpho-spark-usdc-base") || _stringEq(poolId, "morpho-moonwell-flagship-usdc-base")
            || _stringEq(poolId, "fluid-usdc-base") || _stringEq(poolId, "steakhouse-prime-usdc-base");
    }
    ...
    function _jsonRequestDeposit() private view returns (uint256) {
        return
            platform.getRequestDeposit() + SomniaConfig.JSON_API_COST_PER_AGENT * SomniaConfig.DEFAULT_SUBCOMMITTEE_SIZE;
    }

    function _llmRequestDeposit() private view returns (uint256) {
        return platform.getRequestDeposit() + SomniaConfig.LLM_INFERENCE_COST_PER_AGENT
            * SomniaConfig.DEFAULT_SUBCOMMITTEE_SIZE;
    }
